...
gset <- getGEO("GSE48390", GSEMatrix =TRUE, getGPL=FALSE)
length(gset)
# GPL8490
...
risk_class <- as.numeric(p[["predicted risk:ch1"]] == 'high')
status_class <- as.numeric(p[["status:ch1"]] == 'death')
er_class <- as.numeric(p[['er:ch1']] == 'positive')
her2_class<- as.numeric(p[['her2:ch1']] == 'over-expression')
# get time,status for survival analysis
survival_time<-as.numeric(p[["survival time (year):ch1"]])
pid <- gsub('Breast cancer_([0-9A-Z_]+)', '\\1', p$title)
...
df<-cbind(lex,pex)
fwrite(df, file="VisualSHIELD_analysis/GSE48390_all_features_with_max_coefvar_with_status_and_time.csv", sep = ",")
#df<-fread(file="VisualSHIELD_analysis/GSE48390_all_features_with_max_coefvar_with_status.csv", sep = ",")

cdf<-cbind(lex,cex)
fwrite(cdf, file="VisualSHIELD_analysis/GSE48390_all_features_with_max_coefvar_with_status_GSE2034_compatible.csv", sep = ",")

nf<-cbind(lex,nex)
fwrite(nf, file="VisualSHIELD_analysis/GSE48390_all_features_with_max_coefvar_with_status_norm.csv", sep = ",")
#df<-fread(file="VisualSHIELD_analysis/GSE48390_all_features_with_max_coefvar_with_status.csv", sep = ",")

cnf<-cbind(lex,cnex)
fwrite(cnf, file="VisualSHIELD_analysis/GSE48390_all_features_with_max_coefvar_with_status_GSE2034_compatible_norm.csv", sep = ",")
